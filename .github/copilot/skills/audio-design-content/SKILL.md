...
in Norwegian. Policy: **both** a faithful Norwegian transcription and a
separate English translation page, cross-linked to each other.
...
- English page: a full translation, not a summary — this is reference material,
  readers may need the technical detail, not just the gist.
- Link each page to the other near the top ("Norwegian original" / "English
  translation"), and link both from whatever parent page introduces the
  document.
- Formulas, tables, and hand-drawn figures/schematics are identical on both
  pages (formulas as MathJax per the Theory-section convention, figures as
  embedded images) — only the prose is translated.
- This is done as plain sibling content pages (not Hugo's built-in i18n
  multilingual mode) — the Norwegian-language source material is a fixed,
  closed set of old historical documents, not a growing corpus, so the
  overhead of a full multilingual site restructuring (language switcher,
  `content/en/` + `content/no/` trees, menu duplication) isn't warranted.
